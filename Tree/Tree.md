...
A **Balanced Binary Search Tree** is a tree which has a height difference between any two sub trees that is no more that 2 nodes. The height of a sub tree can be found by counting the number of nodes between the root and the leaves. There are many different ways to determine if a tree is balanced, but one of the most common algorithims is known as the Adelson-Velskii and Landis Tree, or **AVL TREE**.
...
If we add the number 4 to the tree, this will add the new node to the left of the node with the value of 5. Now the height of that branch is five, which makes the tree unbalanced.
...
|remove(value)|Remove a value from the tree|O(log n) - Recursively search the tree to find the value, then remove it. May require some cleanup of the neighboring nodes|
...
We use a traversal when we want to display the data in the tree. A forward traversal will list all the nodes from smallest to largest. Similarly, you can also use a reverse traversal to list the nodes from largest to smallest. 

A forward traversal will start on the left subtree of the node, then use the current node, then traverse the right subtree of the node. If the node is empty, then we do not recursively traverse.
